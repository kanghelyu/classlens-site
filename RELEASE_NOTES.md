...
# ClassLens AI v1.0.1 / 课镜 AI

- 版本：v1.0.1（versionCode 30）
- 大小：9,505,355 字节（9.1 MB）

## 本轮改动

1. **周课表自由双向平移**：同一手势的位移同时驱动横竖两个方向（对角线拖动完全跟手），松手带双轴惯性滑动。内容定位仍由官方滚动机制完成，列宽恒定、边界钳制，不再出现「周五缩小 / 列被切」的偏移问题；点击课程格不受影响。
2. **修复手动添加课程保存无效**：点击课表空白格新建时，保存被误判为「更新」而静默丢弃（新 id 不在课表里）。现按 id 是否已存在决定新增/更新，保存必定生效。
3. **空白格新建体验修正**：弹窗标题正确显示「添加课程」，并自动预填所点格子的星期与节次。

# ClassLens AI v1.0.0 / 课镜 AI

最终版：交互打磨收官 + 全局动效统一 + 起始页语言同步。自 v0.9.0 以来的全部改动：

- 版本：v1.0.0（versionCode 29）
- 大小：9,505,372 字节（9.1 MB）

## 本轮改动

- **按钮行全部可横向滚动**：设置/教务/AI 等按钮组与记账 App 一致，窄屏不再挤压。
- **AI 反馈红绿语义**：AI 抓取/识别失败红字、识别成功绿字（全局反馈原则），不再只靠弹窗。
- **底部导航胶囊镜头静止时保持折射**：dock 图标底色平时也带色散，按压增强。
- **全局动效统一常量**：弹层淡入/淡出时长一个地方改，简约统一。
- **起始页语言同步**：选 English 后全校提示与界面即时切英文（修复"选了英文还有中文"根因：S() 全局镜像未同步）。
- 代码清理：移除遗留 AiChatDialog 死代码与废弃 TypeScale 常量。

---

# ClassLens AI v0.9.0 / 课镜 AI

正式版打磨收官：教务多个强老系统适配、AI 交互链路修复、全局模块化与加密打包。

- 版本：v0.9.0（versionCode 28）
- 大小：9,505,376 字节（9.1 MB）
- 加固：R8 全量混淆 + 随机混淆字典 + 类重排去包名 + 源文件名抹除 + 激活密钥异或存储 + 禁云备份（防常规解包：apktool/jadx/strings 无收获）

## 教务系统适配

- **三档浏览器 UA**：安卓（默认）/ 电脑（桌面 Chrome）/ **电脑（Firefox）**。南科大等"仅限电脑"系统实测：Chrome 桌面串白屏、**Firefox 串正常渲染**（tis.sustech.edu.cn 登录页实测通过，Mobile UA 时站点给出"建议 PC 访问"提示属站点自身行为）。老 URP/旧正方换对应档即可。
- 学校清单新增南方科技大学（tis.sustech.edu.cn）；滚动定位、长文本单行滚动、缩放拖移（− / + / pinch）齐全。
- 英文界面：101 所学校全部官方英文名、厂商名拼音+英文、提示与按钮全量英文化。

## AI 交互

- 工具调用轮数上限 4 → 8（修复"翻译课表→选语言→round limit reached"的中断）；写入类工具命中即弹窗预览/确认并提前结束。
- 错误文案友好化；预览/确认弹窗为主，对话内"确认/取消"兜底。

## 全局体验

- 弹层：旋转/横竖屏切换不再自动关闭（configChanges + rememberSaveable 双保险）；统一 CloseButton 组件（× 显式取色，暗色白/亮色黑）；统一 75% 屏高、居中、真液态玻璃。
- 全局字体 Noto Sans SC（子集 2.2MB）；图标 Material Symbols Rounded；按钮/开关/滑杆磨砂随「玻璃磨砂」滑杆线性联动（小控件天然 ×1.6）；透镜模式色散折射全组件齐全（含底部导航胶囊）。
- 首次使用起始页（语言 + 每周第一天）液态玻璃化；数据统一入口（单一 prefs + filesDir/data，旧数据自动迁移）。
- 课表点击空白格自动填写星期/节次；激活/试用系统 fail-open + 防回拨 + 设备绑定激活码（当前免费，服务端未启用收费）。
