(function () {
  ...
  const I18N = {
    en: {
      ...
      feat1Desc: "Every period shows its full time range, weekdays carry their dates, and the grid fits itself \u2014 set up to 30 periods a day and it draws exactly that many, growing further only when a class sits lower down.",
      ...
      feat3Desc: "Pick any theme and course colour by hex, and adjust background blur, scrim, glass material, row height, text size, header dates and end times — from real-time liquid glass to frosted panels.",
      flowTab4: "Desktop AI",
      flowPanel4Kicker: "Hand the timetable to a desktop assistant",
      feat4Title: "Desktop AI access (MCP)",
      feat4Desc: "All 43 schedule tools are exposed as a standard MCP endpoint, so a desktop assistant such as Claude Desktop or WorkBuddy can read and edit the timetable on your phone. Loopback only, bearer token required, writes switchable independently, each write snapshotted first \u2014 and it adds nothing to your notification shade.",
      flowPanel4Note: "Off by default. Open it from Settings \u2192 MCP server; the panel carries the exact commands and configuration to connect.",
      whatsNewSummary: "v1.0.16 \u2014 the AI assistant works again, current models for every provider, any colour for the theme and course cards, and several switches that now do what they say.",
      hardeningNote: "Release builds are signature-checked at runtime; a re-signed package is refused.",
    ...
    zh: {
      ...
      feat1Title: "周课表，一眼看清",
      feat1Desc: "每一节都显示完整的上下课时间段，星期下方带具体日期；网格会自我适配——每天最多 30 节，你开多少就画多少，课程排得更靠后时才自动加长。",
      feat2Title: "三种入口，统一核对流程",
      feat2Desc: "在应用内直接打开学校教务系统登录页，或导入 ICS、CSV、JSON、HTML、XLSX、DOCX 等文件。本地解析，不发送到网络。",
      feat3Title: "液态玻璃外观",
      feat3Desc: "主题色与课程色都能填十六进制选任意颜色；并可调背景模糊、背景遮罩、玻璃材质、行高、字号、顶栏日期、下课时间等——从实时液态玻璃到玻璃磨砂。",
      flowTab4: "桌面端 AI",
      flowPanel4Kicker: "把课表交给桌面助手",
      feat4Title: "桌面端 AI 接口（MCP）",
      feat4Desc: "把全部 43 个课表工具暴露为标准 MCP 接口，桌面端的 Claude Desktop、WorkBuddy 等助手可以直接读取与修改手机上的课表。只监听本机回环地址，每次调用需携带访问密钥；写入可单独关闭，每次写入前自动快照，且不会在通知栏留下任何条目。",
      flowPanel4Note: "默认关闭。入口在「设置 → MCP 服务」，面板内附完整的接入命令与配置。",
      whatsNewSummary: "v1.0.16 —— AI 助手恢复正常、各家模型更新到最新、主题色与课程色可任选，以及多个「点了没反应」的开关修好。",
      hardeningNote: "发布包在运行时校验签名；改包重签的版本会被拒绝。",
      dlKicker: "当前构建",
