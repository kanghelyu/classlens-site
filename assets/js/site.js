(function () {
  ...
  const I18N = {
    en: {
      ...
      feat4Desc: "Every one of the 34 schedule tools is exposed as a standard MCP endpoint, so a desktop assistant such as Claude Desktop or WorkBuddy can read and edit the timetable on your phone. Loopback only, bearer token required, writes switchable independently, and each write is snapshotted first.",
      ...
      whatsNewSummary: "v1.0.13 \u2014 desktop AI access, schedule canvas alignment, import fixes, and a complete English pass.",
    ...
    zh: {
      ...
      feat4Desc: "把全部 34 个课表工具暴露为标准 MCP 接口，桌面端的 Claude Desktop、WorkBuddy 等助手可以直接读取与修改手机上的课表。只监听本机回环地址，每次调用需携带访问密钥；写入可单独关闭，且每次写入前自动快照。",
      flowPanel4Note: "默认关闭。入口在「设置 → MCP 服务」，面板内附完整的接入命令与配置。",
      whatsNewSummary: "v1.0.13 —— 桌面端 AI 接口、课表画布对齐、导入修复，中英翻译补全。",
